...
def build_family() -> autorun.Experiment:
    return autorun.Experiment(
        "research5 trapezoid wsd cooldown",
        {
            "profile": "robustness",
            "phase4": {
...
def main() -> int:
    ...
    if not queue:
        print("No pending research5 WSD batch experiments.")
        return 0
    ...
    with autorun.AUTORUN_LOG.open("a") as log_handle:
        log_handle.write(f"start {time.strftime('%Y-%m-%d %H:%M:%S')} strategy=research5_wsd_batch\n")
        for exp in queue:
