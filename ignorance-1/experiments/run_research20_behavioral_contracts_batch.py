...
FRESH_BLOCK_SEEDS = [107, 108, 109]


def base_phase4() -> dict:
    return {
...
def build_families() -> list[autorun.Experiment]:
    base = base_phase4()
    behavioral = {**base, "phase4_dataset": "behavioral_constraints_v2", "phase4_balance_families": True}
    rigorous = {**behavioral, "phase4_dataset": "behavioral_constraints_v2_rigorous"}
    adversarial = {**behavioral, "phase4_dataset": "behavioral_constraints_v2_adversarial"}
    minimal_pairs = {**base, "phase4_dataset": "semantic_contrast_minimal_pairs_v1", "phase4_balance_families": True}
...
def main() -> int:
    ...
    if not queue:
        print("No pending research20 behavioral contracts batch experiments.")
        return 0
    ...
    with autorun.AUTORUN_LOG.open("a") as log_handle:
        log_handle.write(f"start {time.strftime('%Y-%m-%d %H:%M:%S')} strategy=research20_behavioral_contracts_batch\n")
        for exp in queue:
