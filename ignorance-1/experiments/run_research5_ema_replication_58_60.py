...
FRESH_BLOCK_SEEDS = [58, 59, 60]


def build_family() -> autorun.Experiment:
    return autorun.Experiment(
        "research5 high inertia ema target replication",
        {
            "profile": "robustness",
            "phase4": {
...
def main() -> int:
    ...
    if not queue:
        print("No pending research5 EMA replication experiments.")
        return 0
    ...
    with autorun.AUTORUN_LOG.open("a") as log_handle:
        log_handle.write(f"start {time.strftime('%Y-%m-%d %H:%M:%S')} strategy=research5_ema_replication_58_60\n")
        for exp in queue:
