...
def build_families() -> list[autorun.Experiment]:
    rigorous = base_phase4()
    joint = {**rigorous, "phase4_joint_training": True}
    cc_staged_hard = {
        **joint,
...
def main() -> int:
    ...
    if not queue:
        print("No pending research21 rigorous-edge follow-up experiments.")
        return 0
    ...
    with autorun.AUTORUN_LOG.open("a") as log_handle:
        log_handle.write(f"start {time.strftime('%Y-%m-%d %H:%M:%S')} strategy=research21_rigorous_edge_followup_batch\n")
        for exp in queue:
