...
def main() -> int:
    ...
    parser.add_argument("--output", default=str(ROOT / "artifacts" / "validation_mode_comparison.json"))
    ...
    if requested_device == "cuda" and selected_device != "cuda" and not args.allow_cpu_fallback:
        print(
            f"[val-mode] refusing CPU fallback for requested cuda: {fallback_reason}",
            flush=True,
        )
        return 2
    ...
    print(
        f"[val-mode] requested_device={requested_device} selected_device={selected_device} seeds={seeds}",
        flush=True,
    )
    if fallback_reason is not None:
        print(f"[val-mode] fallback_reason={fallback_reason}", flush=True)

    runs: list[dict[str, object]] = []
    by_seed: dict[int, dict[str, dict[str, object]]] = {}
    for seed in seeds:
        for mode_name, mode_updates in (
            ("training_mode_validation", {"validation_eval_mode": False}),
            ("eval_mode_validation", {"validation_eval_mode": True}),
        ):
            ...
            set_seed(seed)
            print(f"[val-mode] seed={seed} mode={mode_name} starting", flush=True)
            result = run_phase4(
    ...
    for seed in seeds:
        baseline = by_seed[seed]["training_mode_validation"]
        fixed = by_seed[seed]["eval_mode_validation"]
        comparisons.append(
            {
                "seed": seed,
                "pairwise_margin_std_delta": fixed["pairwise_margin_std"] - baseline["pairwise_margin_std"],
                "largest_val_loss_std_delta": fixed["largest_val_loss_std"] - baseline["largest_val_loss_std"],
                "competitor_val_loss_std_delta": fixed["competitor_val_loss_std"] - baseline["competitor_val_loss_std"],
                "pairwise_win_rate_delta": fixed["pairwise_win_rate"] - baseline["pairwise_win_rate"],
                "confidence_margin_delta": fixed["confidence_margin"] - baseline["confidence_margin"],
                "largest_margin_ratio_delta": fixed["largest_margin_ratio"] - baseline["largest_margin_ratio"],
                "training_mode_validation": baseline,
                "eval_mode_validation": fixed,
            }
        )
    ...
    print(f"[val-mode] wrote output={output_path}", flush=True)
    return 0
