"""
v409: Measure exact confidence scores for all 16 eval cases with gate disabled.

We need to know: for the 5 abstaining families, what are the actual reranker scores
(not just conf) when gate is off? And for json_parse-u, what are the detailed scores?
"""
...
# Run with selective gate DISABLED (floor=0.0, mode=none) and very low threshold
r = subprocess.run([PY, "/mnt/Storage/Projects/catbelly_studio/ignorance-1/test_2.7b.py", "15000000", tmp, "--json",
    ...
    "--confidence-threshold", "0.0",  # No conf threshold
    "--lexical-weight", "0.4",
...
print(f"v409 Diagnostic: Gate=disabled, threshold=0.0")
print(f"Score: {score:.2f}")
print()
print(f"{'Family':<15} {'Type':<12} {'Conf':>7} {'Sim':>7} {'Top1':>7} {'Margin':>7} {'Status':<40}")
print("-" * 100)
...
for r2 in sorted(results, key=lambda x: (x.get("family",""), x.get("type",""))):
    ...
    print(f"{fam:<15} {typ:<12} {conf:>7.4f} {sim:>7.4f} {top1:>7.4f} {margin:>7.4f} {status:<40}")
