...
def build_families() -> list[autorun.Experiment]:
    ...
    return [
        ...
        autorun.Experiment(
            ...
            {"profile": "robustness", "phase4": {**ranking_medium}},
        ),
        autorun.Experiment(
        ...
        autorun.Experiment(
            ...
            {"profile": "robustness", "phase4": {**cc_staged_smooth_plus_ranking}},
        ),
    ]
...
def main() -> int:
    ...
    if not queue:
        print("No pending research19 semantic contrast deconfounded batch experiments.")
        return 0
    ...
    with autorun.AUTORUN_LOG.open("a") as log_handle:
        log_handle.write(f"start {time.strftime('%Y-%m-%d %H:%M:%S')} strategy=research19_semantic_contrast_deconfounded_batch\n")
        for exp in queue:
