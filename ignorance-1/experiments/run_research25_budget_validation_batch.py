...
def build_families() -> list[autorun.Experiment]:
    ...
    return [
        ...
        autorun.Experiment(
            ...
            {"profile": "robustness", "phase4": {**high_split}},
        ),
    ]
...
def main() -> int:
    ...
    if not queue:
        print("No pending research25 budget-validation experiments.")
        return 0
    ...
    with autorun.AUTORUN_LOG.open("a") as log_handle:
        log_handle.write(f"start {time.strftime('%Y-%m-%d %H:%M:%S')} strategy=research25_budget_validation_batch\n")
        for exp in queue:
