...
def build_families() -> list[autorun.Experiment]:
    ...
    return [
        ...
        autorun.Experiment(
            "research7 common randomness accum4",
            {
                "profile": "robustness",
                "phase4": {
                    **base,
        ...
        autorun.Experiment(
            "research7 common randomness accum4 anchor150m",
            {
                "profile": "robustness",
                "phase4": {
                    **base,
...
def main() -> int:
    ...
    if not queue:
        print("No pending research7 protocol batch experiments.")
        return 0
    ...
    with autorun.AUTORUN_LOG.open("a") as log_handle:
        log_handle.write(f"start {time.strftime('%Y-%m-%d %H:%M:%S')} strategy=research7_protocol_batch\n")
        for exp in queue:
