    ...
    return [
        ...
        autorun.Experiment(
            "research9 alignment prediction heavy",
            {
                ...
                "phase4": {
                    **base,
                    "alignment_prediction_weight": 1.5,
                    "alignment_embedding_weight": 0.25,
...
def main() -> int:
    ...
    if not queue:
        print("No pending research9 alignment ablation batch experiments.")
        return 0
    ...
    with autorun.AUTORUN_LOG.open("a") as log_handle:
        log_handle.write(f"start {time.strftime('%Y-%m-%d %H:%M:%S')} strategy=research9_alignment_ablation_batch\n")
        for exp in queue:
