...
def main() -> int:
    ...
    if not queue:
        print("No pending research11 compute fair batch experiments.")
        return 0
    ...
    with autorun.AUTORUN_LOG.open("a") as log_handle:
        log_handle.write(f"start {time.strftime('%Y-%m-%d %H:%M:%S')} strategy=research11_compute_fair_batch\n")
        for exp in queue:
