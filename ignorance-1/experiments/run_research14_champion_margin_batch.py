...
def main() -> int:
    ...
    if not queue:
        print("No pending research14 champion margin batch experiments.")
        return 0
    ...
    with autorun.AUTORUN_LOG.open("a") as log_handle:
        log_handle.write(f"start {time.strftime('%Y-%m-%d %H:%M:%S')} strategy=research14_champion_margin_batch\n")
        for exp in queue:
