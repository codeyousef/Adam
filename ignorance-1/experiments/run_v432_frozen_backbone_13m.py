#!/usr/bin/env python3
"""
v432: Replicate v378 incumbent config at 13.6M with freeze_backbone=true.

CRITICAL FINDING from v431: ALL 30 seeds collapsed at 13.6M with freeze_backbone=false.
The v378 incumbent used:
  1. freeze_backbone=true (backbone frozen, only heads trained)
  2. query_multiview=true
  3. vicreg_weight=0.02, rank_reg_weight=0.08 (anti-collapse regularizers)
  4. late_interaction_verifier_mode=hard_maxsim

The current phase4.py now supports freeze_backbone=true.
This batch tests 10 seeds with the v378 incumbent config at 13.6M.
If any pass → the config is replicable with multi-seed selection + frozen backbone
If all fail → the v378 PASS came from the multi-cycle selection, not just config
"""
...
def make_config(seed):
    return {
        "phase4": {
            ...
            # KEY: freeze backbone (only train heads)
...
def main():
    print("="*70)
    print("v432: v378 incumbent config (freeze_backbone=true) @ 13.6M")
    ...
    print("\n" + "="*70)
    print("v432 RESULTS — v378 incumbent config @ 13.6M")
    print("="*70)
