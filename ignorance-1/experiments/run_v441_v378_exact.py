#!/usr/bin/env python3
"""
v441: EXACT v378 config replication.
The v378 cycle-0 (late-inter-only-seed511-seed509) achieved offdiag=0.030.
Key config: freeze_backbone=True, warm_start_phase3_only=True (from v338),
use_retrieval_data_strategy=True, retrieval_facet_loss_weight=0.35,
ranking_margin_weight=0.20, same_family_only_ranking=True.
"""
...
V338_CKPT = "/mnt/Storage/Projects/catbelly_studio/ignorance-1/artifacts/strict_eval_autoresearch_v338/v338-promoted-earlier-onset-tiny-mixed-bridge-seed504/model.pt"
...
V441_CONFIG = {
    ...
    "phase4": {
        ...
        # NO warm-start (v338 has wrong architecture shape)
        # "warm_start_model_path": V338_CKPT,
        # "warm_start_phase3_only": True,
        # Freeze backbone
        "freeze_backbone": True,
        # Optimizer — match v378
        ...
        # Retrieval heads — THE KEY COMPONENTS
        ...
        # Retrieval data strategy — hard negatives
...
def train_one(seed):
    label = "v378exact"
    out_dir = f"{OUT_BASE}/v441-{label}-seed{seed}"
    ...
    cfg = {"seed": seed, "device": "cuda", "profile": "4090",
           "phase4": dict(V441_CONFIG["phase4"], seed=seed),
...
def eval_one(model_path, seed):
    eval_path = f"{OUT_BASE}/v441-v378exact-seed{seed}/eval.json"
...
def main():
    print("="*60)
    print("v441: EXACT v378 config replication (with warm-start)")
    print("  freeze_backbone=True, warm_start_phase3_only=True")
    print("  use_retrieval_data_strategy=True, retrieval_facet_loss_weight=0.35")
    print("  same_family_only_ranking=True, ranking_margin_weight=0.20")
    print("="*60)

    seeds = [511]
    for seed in seeds:
        out_dir = f"{OUT_BASE}/v441-v378exact-seed{seed}"
