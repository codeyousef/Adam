"""
Adam autoresearch training script (Track B). Single-GPU, single-file.
...
Base: Qwen2.5-Coder-3B-Instruct with QLoRA (4-bit NF4)
...
from prepare import (
    TIME_BUDGET, DEVICE, MAX_SEQ_LEN,
    load_base_model, evaluate_pi, TARGETS,
)
...
N_L1 = 3500                 # context override is saturated; free budget for L2
N_L2 = 5500                 # physics counterfactual examples need the most help
N_L3 = 5000                 # keep the current strong L3 mix stable
N_L4 = 3000                 # keep the separator-based L4 improvement intact
...
def train():
    ...
    print(f"Model: {n_total / 1e6:.1f}M total, {n_train / 1e6:.1f}M trainable on {DEVICE}")
